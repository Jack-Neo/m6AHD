...
auc_results <- data.frame(Combination = character(0), AUC = numeric(0))

# 循环训练每个特征组合
for (i in 1:63) {
  # 获取当前特征组合
  cb_n <- feature_combinations_n[i]
  cb_n <- as.data.frame(cb_n)
  cb_p <- feature_combinations_p[i]
  ...
  train_control <- trainControl(
    ...
    summaryFunction = twoClassSummary,  # 使用二分类问题的评估指标（如AUC, Sensitivity     等）
  ...
  # 将AUC结果存入数据框
  auc_results <- rbind(auc_results, data.frame(Combination = paste("Combination", i), AUC = auc_value))
}

# 将AUC结果写入xlsx文件
write.xlsx(auc_results, "D:\\Users\\Administrator\\Desktop\\auc_results.xlsx")
