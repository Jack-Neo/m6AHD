...
# 获取当前特征组合
cb_n <- feature_combinations_n[58]
cb_n <- as.data.frame(cb_n)
cb_p <- feature_combinations_p[58]
...
train_features <- train_features[, -(585:1125)]
train_features <- train_features[, -(585:844)]
test_features <- test_data[, !(colnames(test_data) == "type")]
test_features <- test_features[, -(585:1125)]
test_features <- test_features[, -(585:844)]
test_labels <- test_data$type
test_features_Calcific_up=test_features
test_labels_Calcific_up=test_labels
saveRDS(test_features_Calcific_up,file = "D:\\Users\\Administrator\\Desktop\\machine learning\\feature\\test_features_Calcific_up.rds")
saveRDS(test_labels_Calcific_up, file = "D:\\Users\\Administrator\\Desktop\\machine learning\\label\\test_labels_Calcific_up.rds")



# 确保 train_labels 是因子型变量（用于分类问题）
...
rf_model <- randomForest(
  ...
  y = train_labels,         # 输入标签
  ntree = 100,        # 设置 ntree 参数
  mtry = 113,          # 设置 mtry 参数
  importance = TRUE,        # 计算特征的重要性
  type = "classification"   # 确保是分类任务
)

# 计算预测概率
prob <- predict(rf_model, test_features, type = "prob")  # 返回每一类的概率

# 确保 test_labels 是测试集的标签
# 计算AUROC，假设标签是二分类任务
# 对于二分类问题，我们选择正类的概率作为评估标准
roc_curve <- roc(test_labels, prob[, 2])  # 使用测试集标签
auroc <- auc(roc_curve)  # 计算AUROC值
auroc
model_Calcific_up=rf_model
saveRDS(model_Calcific_up, file = "D:\\Users\\Administrator\\Desktop\\machine learning\\model\\model_Calcific_up.rds")


prob <- predict(rf_model, test_features, type = "prob")[,2]

# 构建作图用数据框
df <- data.frame(
  d = factor(test_labels),
  m = prob
)
setwd("D:\\Users\\Administrator\\Desktop\\machine learning\\new\\Calcific\\up")
write.csv(df, "roc_data.csv", row.names = FALSE)
