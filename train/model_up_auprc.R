...
#C=n,T=p
max_length <-  81
auc_results <- data.frame(Model = character(0), AUC = numeric(0), AUPRC = numeric(0))
C <- readRDS("D:\\Users\\Administrator\\Desktop\\machine learning\\new\\AC16EV\\up\\AC16EV_UP_n_m6A.rds")
T<- readRDS("D:\\Users\\Administrator\\Desktop\\machine learning\\new\\AC16EV\\up\\AC16EV_UP_p_m6A_s.rds")
...
adjusted_granges <- GRanges(
  seqnames = seqnames(C),
  ranges = IRanges(
    start = start_positions - (max_length-1)/2+2,  # 将起始位置调整为原位置前个碱基
    end = end_positions + (max_length-1)/2-2       # 将结束位置调整为原位置后个碱基
  ),
  strand = strand(C)
)
...
# 获取当前特征组合
cb_n <- feature_combinations_n[42]
cb_n <- as.data.frame(cb_n)
cb_p <- feature_combinations_p[42]
...
svm_model <- train(
  ...
  y = train_labels,          # 输入标签
  method = "svmRadial",      # SVM径向基核
  trControl = train_control, # 训练控制
  probability = TRUE         # 启用概率估计
)

# 预测结果
predictions_prob <- predict(svm_model, test_features, type = "prob")

# 计算AUC
roc_curve <- roc(test_labels, predictions_prob[, 2])
auc_value <- auc(roc_curve)
# --- 新增: 计算 AUPRC ---
fg <- predictions_prob[test_labels == "p", "p"]
bg <- predictions_prob[test_labels == "n", "p"]
# 计算 PR 曲线面积
pr <- pr.curve(scores.class0 = fg, scores.class1 = bg, curve = TRUE)
auprc_value <- pr$auc.integral

auc_results <- rbind(auc_results, data.frame(Model = "SVM", AUC = as.numeric(auc_value), AUPRC = auprc_value))




predictions <- factor(predictions, levels = c("n", "p"))
test_labels <- factor(test_labels, levels = c("n", "p"))
# 计算混淆矩阵
cm <- confusionMatrix(predictions, test_labels)
# 提取Accuracy, Sensitivity, Specificity
...
# 保存结果
auc_results <- rbind(auc_results, data.frame(Model = "RF", AUC = as.numeric(auc_value), AUPRC = auprc_value))
...
logistic_model <- train(
  ...
  method = "glm",                 # 使用逻辑回归
  trControl = train_control,      # 训练控制
  family = "binomial"             # 指定逻辑回归的家庭（二项式）
)

# 使用训练好的模型进行预测
predictions <- predict(logistic_model, test_features, type = "raw")
predictions_prob <- predict(logistic_model, test_features, type = "prob")

# --- 计算 AUC (ROC) ---
roc_curve <- roc(test_labels, predictions_prob[, "p"])
auc_value <- auc(roc_curve)
...
# 保存结果
auc_results <- rbind(auc_results, data.frame(Model = "GLM", AUC = as.numeric(auc_value), AUPRC = auprc_value))
...
# 将数据转换为xgboost所需的格式
train_features_matrix <- as.matrix(train_features)  # 转换为矩阵
test_features_matrix <- as.matrix(test_features)    # 转换为矩阵
# 假设 train_labels 是一个字符型变量，包含 "n" 和 "p"
train_labels_binary <- as.numeric(factor(train_labels, levels = c("n", "p"))) - 1

# 假设 train_labels 是一个字符型变量，包含 "n" 和 "p"
test_labels_binary <- as.numeric(factor(test_labels, levels = c("n", "p"))) - 1
dtrain <- xgb.DMatrix(data = train_features_matrix, label = train_labels_binary)
# 使用默认参数训练
params <- list(
  objective = "binary:logistic",  # 二分类问题
  eval_metric = "logloss"  # 评估标准为对数损失（log loss）
)
# 设置训练的轮次（迭代次数）
nrounds <- 10
...
pred_probs <- predict(model, test_features_matrix)
roc_curve <- roc(test_labels_binary, pred_probs)
auc_value <- auc(roc_curve)

# --- 新增: 计算 AUPRC ---
# XGBoost 中 test_labels_binary: p是1, n是0
fg <- pred_probs[test_labels_binary == 1] # 正类概率
bg <- pred_probs[test_labels_binary == 0] # 负类概率

pr <- pr.curve(scores.class0 = fg, scores.class1 = bg, curve = TRUE)
auprc_value <- pr$auc.integral

# 保存结果
auc_results <- rbind(auc_results, data.frame(Model = "xgboost", AUC = as.numeric(auc_value), AUPRC = auprc_value))
pred_labels <- ifelse(pred_probs > 0.5, 1, 0)
conf_matrix <- confusionMatrix(factor(pred_labels, levels = c(0, 1)), 
                               factor(test_labels_binary, levels = c(0, 1)))
# 提取混淆矩阵中的准确率（Accuracy）、敏感性（Sensitivity）和特异性（Specificity）
