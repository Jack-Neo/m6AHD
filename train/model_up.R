...
#C=n,T=p
max_length <-  41
auc_results <- data.frame(Model = character(0), AUC = numeric(0))
C <- readRDS("D:\\Users\\Administrator\\Desktop\\machine learning\\new\\hypertrophy\\up\\hypertrophy_UP_n_m6A.rds")
T<- readRDS("D:\\Users\\Administrator\\Desktop\\machine learning\\new\\hypertrophy\\up\\hypertrophy_UP_p_m6A_s.rds")
...
# 获取当前特征组合
cb_n <- feature_combinations_n[26]
cb_n <- as.data.frame(cb_n)
cb_p <- feature_combinations_p[26]
...
auc_results <- rbind(auc_results, data.frame(Model = paste("SVM"), AUC = auc_value))
...
# 计算ROC曲线
roc_curve <- roc(test_labels, predictions_prob[, 2])
auc_results <- rbind(auc_results, data.frame(Model = paste("RF"), AUC = auc(roc_curve)))
...
# 计算ROC曲线
roc_curve <- roc(test_labels, predictions_prob[, 2])
auc_results <- rbind(auc_results, data.frame(Model = paste("GLM"), AUC = auc(roc_curve)))
predictions <- factor(predictions, levels = c("n", "p"))
test_labels <- factor(test_labels, levels = c("n", "p"))
# 计算混淆矩阵
cm <- confusionMatrix(predictions, test_labels)
...
auc_results <- rbind(auc_results, data.frame(Model = paste("xgboost"), AUC = auc(roc_curve)))
