...
# 定义 ntree 和 mtry 的值范围
ntree_values <- seq(100, 1500, by = 100)#seq(10, 1000, by = 2)  # 可以根据需要调整ntree的值
mtry_values <- seq(30, 350, by = 1)# 可以根据需要调整mtry的值
...
for (ntree_val in ntree_values) {
  for (mtry_val in mtry_values) {
    ...
    rf_model <- randomForest(
      x = train_features,       # 输入特征
      y = train_labels,         # 输入标签
      ntree = ntree_val,        # 设置 ntree 参数
      mtry = mtry_val,          # 设置 mtry 参数
    ...
    # 将结果存储到结果数据框中
    results <- rbind(results, data.frame(ntree = ntree_val, mtry = mtry_val, AUROC = auroc))
  }
}

results

# 找到最佳的 ntree 和 mtry 组合
best_combination <- results[which.max(results$AUROC), ]
print(best_combination)

library(openxlsx)
write.xlsx(results, "D:\\Users\\Administrator\\Desktop\\machine learning\\new\\TKIs\\up\\auc_parmetter_2.xlsx")